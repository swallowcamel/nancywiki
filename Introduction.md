...
1. Grab the ASP.NET host for Nancy `install-package Nancy.Hosting.Aspnet` (this will also install the `Nancy` nuget and update the `web.config` file so that Nancy is made the handler of any request)
